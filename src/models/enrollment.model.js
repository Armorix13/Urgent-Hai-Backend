...
enrollmentSchema.statics.getUserEnrollments = async function (userId) {
  ...
  return await this.find({
    ...
    .populate({
      path: "course",
      select:
        "title identifierId description thumbnail duration level category courseType price courseContent isDeleted tags benefits prerequisites learningOutcomes isActive rating enrollmentCount",
    })
    .sort({ enrolledAt: -1 })
    .lean();
};
